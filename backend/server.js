...
app.get('/api/search/tracks', async function(req, res) {
    ...
    try {
        ...
        console.error(err);
        res.status(500).json({ error: 'Erreur recherche titres' });
    }
});

// ── Fiche artiste (bio Wikipedia + top 5 Last.fm + iTunes) ───────
app.get('/api/search/artist', async function(req, res) {
    ...
    try {
        // 1. Bio Last.fm
        const infoUrl  = `https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${encodeURIComponent(name)}&api_key=${LASTFM_KEY}&format=json&lang=fr`;
        // 2. Top tracks Last.fm
        ...
        // Bio — nettoyer le HTML et couper à 300 chars
        let bio = '';
        ...
        // Image artiste via iTunes
        const image = await getArtistImageFromItunes(name);

        // Top 5 titres enrichis avec iTunes
        ...
        console.error(err);
        res.status(500).json({ error: 'Erreur fiche artiste' });
    }
});

// ── Helper : image artiste via iTunes ────────────────────────────
async function getArtistImageFromItunes(artistName) {
    // 1. Essayer Last.fm en priorité (vraie photo artiste)
    try {
        ...
        // Last.fm retourne plusieurs tailles, on prend "extralarge"
    ...
    // 2. Fallback iTunes (pochette d'album)
    try {
        const url  = `https://itunes.apple.com/search?term=${encodeURIComponent(artistName)}&media=music&entity=musicArtist&limit=1`;
        const res  = await fetch(url);
        const data = await res.json();
        if (data.results && data.results.length > 0) {
